import type { AvailabilityState, MenuItem } from '@/lib/api';
...
export interface ItemSizeOption {
  ...
  name: string;
  price: number;
  /** Computed per-size availability, stamped by the menu service. A "sold_out"
   *  size can't be ordered — the order-time guard rejects it — so pickers must
   *  disable it. Undefined on items that aren't per-size stock-tracked. */
  availabilityState?: AvailabilityState;
}

/** Whether a size choice is currently orderable (not stamped sold-out). */
export function isSizeOrderable(o: ItemSizeOption): boolean {
  return o.availabilityState !== 'sold_out' && o.availabilityState !== 'hidden';
}
...
export function itemSizeOptions(item: MenuItem): ItemSizeOption[] {
  ...
  if (opts.length > 0) {
    return opts.map((o) => ({
      id: o.id,
      name: o.name,
      price: o.price > 0 ? o.price : item.price,
