      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-0 mt-1 text-sm text-fg-tertiary">
              {restaurant?.name && (
                <>
  ...
  return (
    <tr className="border-b border-[var(--divider)] hover:bg-[var(--surface-subtle)] transition-colors cursor-pointer" onClick={() => router.push(`/${rid}/menu/items/${item.id}`)}>
      ...
      <td className="py-3 px-4">
        <div className="flex items-center gap-2">
          {item.image_url ? (
            <img src={item.image_url} alt="" className="w-9 h-9 rounded object-cover shrink-0" />
          ) : (
      ...
      <td className="py-3 px-4 text-fg-secondary text-xs">{modifierNames}</td>
