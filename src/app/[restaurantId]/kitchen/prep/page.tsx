    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="card p-4">
          <p className="text-xs text-fg-secondary uppercase tracking-wider">Prep Items</p>
        ...
        <div className="card p-4">
          <p className="text-xs text-fg-secondary uppercase tracking-wider">Categories</p>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 max-w-xs">
          ...
          <input type="text" placeholder="Search prep items..." value={search} onChange={(e) => setSearch(e.target.value)} className="input pl-9 pr-3 py-2 text-sm w-full" />
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="card text-center py-16 space-y-3">
          <p className="text-lg font-semibold text-fg-primary">No prep items found</p>
          <p className="text-sm text-fg-secondary">
            {items.length === 0 ? 'Add your first prep recipe to get started.' : 'Try adjusting your filters.'}
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs text-fg-secondary uppercase tracking-wider" style={{ borderBottom: '1px solid var(--divider)' }}>
                ...
                <th className="py-3 px-4 font-medium text-right">Yield/Batch</th>
                <th className="py-3 px-4 font-medium text-right">Cost/Unit</th>
                <th className="py-3 px-4 font-medium text-right">Shelf Life</th>
                  ...
                  <tr key={item.id} className="hover:bg-[var(--surface-subtle)] transition-colors" style={{ borderBottom: '1px solid var(--divider)' }}>
                    ...
                    <td className="py-3 px-4">
                      {isLow ? (
                        <span className="flex items-center gap-1 text-red-500 text-xs font-medium">
                          <ExclamationTriangleIcon className="w-4 h-4" /> Low
                        </span>
                      ) : (
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-1">
                        <button onClick={() => setBatchModal({ open: true, item })} className="p-1 rounded hover:bg-[var(--surface-subtle)]" title="Produce batch">
                          <PlayIcon className="w-4 h-4 text-fg-secondary" />
                        </button>
                        <button onClick={() => setRecipeModal({ open: true, item })} className="p-1 rounded hover:bg-[var(--surface-subtle)]" title="Edit recipe">
                          <BeakerIcon className="w-4 h-4 text-fg-secondary" />
                        </button>
                        <button onClick={() => setTxModal({ open: true, item })} className="p-1 rounded hover:bg-[var(--surface-subtle)]" title="Waste/Adjust">
                          <ArrowsRightLeftIcon className="w-4 h-4 text-fg-secondary" />
                        </button>
                        <button onClick={() => setItemModal({ open: true, editing: item })} className="p-1 rounded hover:bg-[var(--surface-subtle)]" title="Edit">
                          <PencilIcon className="w-4 h-4 text-fg-secondary" />
                        </button>
                        <button onClick={() => handleDelete(item.id)} className="p-1 rounded hover:bg-[var(--surface-subtle)]" title="Delete">
...
function PrepItemModal({
  ...
  return (
    <Modal title={editing ? 'Edit Prep Item' : 'Add Prep Item'} onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="text-xs text-fg-secondary block mb-1">Current Stock</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs text-fg-secondary block mb-1">Yield per Batch</label>
          ...
          <div>
            <label className="text-xs text-fg-secondary block mb-1">Shelf Life (hours)</label>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="rounded-modal shadow-xl p-6 w-full max-w-lg mx-4 max-h-[85vh] overflow-y-auto" style={{ background: 'var(--surface)' }}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-fg-primary">Recipe: {item.name}</h3>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            <p className="text-sm text-fg-secondary">Raw ingredients needed to produce one batch ({item.yield_per_batch} {item.unit}):</p>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button onClick={handleSave} disabled={saving} className="btn-primary text-sm">{saving ? 'Saving...' : 'Save Recipe'}</button>
            </div>
          </div>
        )}
      </div>
...
function BatchProduceModal({
  ...
  return (
    <Modal title={`Produce: ${item.name}`} onClose={onClose}>
      <div className="space-y-4">
        <div>
          <label className="text-xs text-fg-secondary block mb-1">Number of Batches</label>
          ...
          <p className="text-xs text-fg-secondary mt-1">
            Will produce: {(batches * item.yield_per_batch).toFixed(1)} {item.unit}
          </p>
        </div>
        ...
        {!preview ? (
          <div className="flex justify-end gap-2">
            <button onClick={onClose} className="btn-secondary text-sm">Cancel</button>
            <button onClick={handlePreview} disabled={loading} className="btn-primary text-sm">{loading ? 'Checking...' : 'Preview'}</button>
          </div>
        ) : (
          <>
            <div className="space-y-2">
              <p className="text-sm font-medium text-fg-primary">Ingredients to consume:</p>
            ...
            {preview.insufficient.length > 0 && (
              <div className="bg-red-500/10 rounded-lg p-3 space-y-1">
                <p className="text-sm font-medium text-red-500 flex items-center gap-1">
                  <ExclamationTriangleIcon className="w-4 h-4" /> Insufficient stock:
                </p>
                {preview.insufficient.map((s) => (
                  <p key={s.stock_item_id} className="text-sm text-red-400">
                    {s.stock_item_name}: need {s.required.toFixed(2)}, have {s.available.toFixed(2)}
                  </p>
                ))}
              </div>
            )}
            ...
            <div className="flex justify-end gap-2">
              ...
              <button onClick={handleProduce} disabled={loading || preview.insufficient.length > 0} className="btn-primary text-sm">
                {loading ? 'Producing...' : 'Confirm Produce'}
              </button>
            </div>
          </>
        )}
...
function PrepTxModal({
  ...
  return (
    <Modal title={`Adjust: ${item.name}`} onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="flex gap-2">
          {(['waste', 'adjust'] as PrepTransactionType[]).map((t) => (
            <button
              key={t}
              type="button"
              onClick={() => setType(t)}
              className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
                type === t ? 'bg-brand-500 text-white' : 'bg-[var(--surface-subtle)] text-fg-secondary'
              }`}
            >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="rounded-modal shadow-xl p-6 w-full max-w-2xl mx-4 max-h-[85vh] overflow-y-auto" style={{ background: 'var(--surface)' }}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-fg-primary">Daily Prep Plan</h3>
          <button onClick={onClose} className="text-fg-secondary hover:text-fg-primary text-xl leading-none">&times;</button>
        </div>
