    ...
    <div className="mx-auto max-w-[840px] space-y-8">
      ...
      <section>
        ...
        <div className="mt-4 divide-y divide-[var(--line)] border-y border-[var(--line)]">
          {JOB_TYPES.map(({ type, title, desc, icon: Icon }) => {
            const active = editor.job_types.includes(type);
            return <PrinterProfileJobSection key={type} type={type} title={t(title)} description={t(desc)} icon={Icon} active={active} onToggle={(checked) => toggleJob(type, checked)}>
                {type === 'receipts' && <ReceiptOptions editor={editor} t={t} onChange={onChange} />}
                {type === 'dine_in_tickets' && <>
                  <CategorySelector ids={editor.dine_in_category_ids} categories={categories} t={t} onChange={(ids) => onChange({ ...editor, dine_in_category_ids: ids })} />
                  <TicketFormatOptions editor={editor} t={t} onChange={onChange} />
                </>}
                {type === 'online_tickets' && <>
                  <CategorySelector ids={editor.online_category_ids} categories={categories} t={t} onChange={(ids) => onChange({ ...editor, online_category_ids: ids })} />
                  <TicketFormatOptions editor={editor} t={t} onChange={onChange} />
                </>}
...
function ReceiptOptions({ editor, t, onChange }: { editor: ProfileEditorState; t: T; onChange: (value: ProfileEditorState) => void }) {
  return <>
...
function CopiesOption({ editor, t, onChange }: { editor: ProfileEditorState; t: T; onChange: (value: ProfileEditorState) => void }) {
  return <div className="flex items-center justify-between gap-5 py-4"><span className="text-fs-sm font-semibold">{t('printerProfileCopies')}</span><div className="flex items-center rounded-full border border-[var(--line)] bg-[var(--surface)] p-1"><Button variant="ghost" size="sm" icon aria-label={t('decrease')} disabled={editor.copies <= 1} onClick={() => onChange({ ...editor, copies: Math.max(1, editor.copies - 1) })}><Minus /></Button><span className="min-w-10 text-center text-fs-sm font-semibold tabular-nums">{editor.copies}</span><Button variant="ghost" size="sm" icon aria-label={t('increase')} disabled={editor.copies >= 5} onClick={() => onChange({ ...editor, copies: Math.min(5, editor.copies + 1) })}><Plus /></Button></div></div>;
}
